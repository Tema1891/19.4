...
def test_add_new_pet_with_valid_data(name='Gigant', animal_type='dog',
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Суперкот", "кот","2", "images/amer.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_non_valid_login(email=non_valid_email, password=valid_password):
    """ Проверяем что запрос api ключа возвращает статус 403 и в результате содержится ответ,
...
def test_add_photo_exist_pet(pet_photo='images/307.jpg'):
    """Проверяем что можно добавить фото питомца к уже существующему животному в базе данных
     для этого отправляем запрос на добавление фотографии и получаем в ответ код 200 о том что операция выполнена
